...
@pytest.fixture
def mock_crud_cart():
    with patch("app.api.v1.public.cart.crud_cart") as mock:
        yield mock

@pytest.fixture
def mock_db_session():
    with patch("app.api.v1.public.cart.get_db") as mock:
        yield mock
...
def mock_db_queries():
    ...
    with patch("app.api.v1.public.cart.Session.query") as mock_query:
        yield mock_query

# 1. Guest cart creation
# 3. Add item
# 17. Cookie behavior
@patch("app.api.v1.public.cart._build_cart_response")
...
from app.api.v1.public.cart import _build_cart_response
